...
class APIEntriesTests(unittest.TestCase):
    ...
    def test_commit_entry(self):
        ...
        # commit chain
        commit_success, commit_response = self.api_objects_factomd.commit_chain(message)
        if not commit_success:
            ...
            # reveal chain
            reveal_success, reveal_response = self.api_objects_factomd.reveal_chain(entry)
            if not reveal_success:
                ...
                chainid = str(reveal_response['chainid'])
                entry_external_ids, content = generate_random_external_ids_and_content()

                #compose entry
                message, entry = self.api_objects_wallet.compose_entry(chainid, entry_external_ids, content, self.entry_credit_address1000)
    ...
    def test_repeated_commits(self):
        ...
        # commit chain
        message, entry, external_ids = self.__compose_chain()
        commit_success, commit_response = self.api_objects_factomd.commit_chain(message)
    ...
    def test_reveal_chain(self):
        # commit chain
        message, entry, external_ids = self.compose_chain()
        commit_success, commit_response = self.api_objects_factomd.commit_chain(message)
        if not commit_success:
            fail_message, info, entryhash = self.__failure_data(commit_response)
            self.assertTrue(False, 'Chain commit failed - ' + fail_message + ' - ' + info + ' - entryhash ' + entryhash)
