...
class APITestsHeights(unittest.TestCase):
    ...
    def test_check_admin_block_height(self):
        blocks = self.api_factomd.get_heights()
        ...
        for factomd_address_custom in self.factomd_address_custom_list:
            for x in range(0, int(directory_block_height)):
                ...
                admin_block_height = self.api_factomd.get_admin_block_by_height(x)
                self.api_factomd.change_factomd_address(factomd_address_custom)
                admin_block_height_1 = self.api_factomd.get_admin_block_by_height(x)
    ...
    def test_check_directory_block_height(self):
        ...
        for factomd_address_custom in self.factomd_address_custom_list:
            for x in range(0, int(directory_block_height)):
                self.api_factomd.change_factomd_address(self.factomd_address)
                dblock_height = self.api_factomd.get_directory_block_by_height(x)
                self.api_factomd.change_factomd_address(factomd_address_custom)
                dblock_height_1 = self.api_factomd.get_directory_block_by_height(x)
    ...
    def test_check_entrycredit_block_height(self):
        ...
        for factomd_address_custom in self.factomd_address_custom_list:
            for x in range(0, int(directory_block_height)):
                self.api_factomd.change_factomd_address(self.factomd_address)
                entrycredit_block_height = self.api_factomd.get_entry_credit_block_by_height(x)
                self.api_factomd.change_factomd_address(factomd_address_custom)
                entrycredit_block_height_1 = self.api_factomd.get_entry_credit_block_by_height(x)
    ...
    def test_check_factoid_block_height(self):
        ...
        for factomd_address_custom in self.factomd_address_custom_list:
            for x in range(0, int(directory_block_height)):
                self.api_factomd.change_factomd_address(self.factomd_address)
                factoid_block_height = self.api_factomd.get_factoid_block_by_height(x)
                self.api_factomd.change_factomd_address(factomd_address_custom)
                factoid_block_height_1 = self.api_factomd.get_factoid_block_by_height(x)
    ...
    def test_check_factoid_block_keymr(self):
        ...
        for factomd_address_custom in self.factomd_address_custom_list:
            for x in range(0, int(directory_block_height)):
                ...
                fblock_keymr_result = self.api_factomd.get_factoid_block_by_keymr(fblock_keymr)
                factoid_block_height = self.api_factomd.get_factoid_block_by_height(x)
                self.assertTrue(factoid_block_height == fblock_keymr_result,
                                "mismatch in call to factoid block keymr at height %d" % (x))

                self.api_factomd.change_factomd_address(factomd_address_custom)
                fblock_keymr_result_1 = self.api_factomd.get_factoid_block_by_keymr(fblock_keymr)
    ...
    def test_check_entrycredit_block_keymr(self):
        ...
        for factomd_address_custom in self.factomd_address_custom_list:
            for x in range(0, int(directory_block_height)):
                ...
                ecblock_keymr_result = self.api_factomd.get_entrycredit_block_by_keymr(ecblock_keymr)
                factoid_block_height = self.api_factomd.get_entry_credit_block_by_height(x)
                self.assertTrue(factoid_block_height == ecblock_keymr_result,
                                "mismatch in call to entrycredit block keymr at height %d" % (x))

                self.api_factomd.change_factomd_address(factomd_address_custom)
                ecblock_keymr_result_1 = self.api_factomd.get_entrycredit_block_by_keymr(ecblock_keymr)
    ...
    def test_check_admin_block_keymr(self):
        ...
        for factomd_address_custom in self.factomd_address_custom_list:
            for x in range(0, int(directory_block_height)):
                ...
                ablock_keymr_result = self.api_factomd.get_admin_block_by_keymr(ablock_keymr)
                admin_block_height = self.api_factomd.get_admin_block_by_height(x)
                self.assertTrue(admin_block_height == ablock_keymr_result,
                                "mismatch in call to admin block keymr at height %d" % (x))

                self.api_factomd.change_factomd_address(factomd_address_custom)
                ablock_keymr_result_1 = self.api_factomd.get_admin_block_by_keymr(ablock_keymr)
