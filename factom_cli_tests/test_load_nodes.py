...
class FactomLoadNodes(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.entry_credit_address100000 = fund_entry_credit_address(100000)

    def test_make_chain_and_check_balance(self):
        ENTRIES_PER_BLOCK = 4000
        print 'sleep time', float(self.data['BLOCKTIME']) / float(ENTRIES_PER_BLOCK)
        self.factom_cli_create.check_wallet_address_balance(self.entry_credit_address100000)
        ...
        for i in xrange(10):
            ...
            for i in range(80):
                ...
                chain_id = self.factom_chain_object.make_chain_from_binary_file(self.entry_credit_address100000, path,
                ...
                for i in range(120):
                    ...
                    self.factom_chain_object.add_entry_to_chain(self.entry_credit_address100000, path, names_list,
                                                                flag_list=chain_flags_list)
                    # time.sleep(float(self.data['BLOCKTIME']) / float(ENTRIES_PER_BLOCK) * 0.2)
                    self.factom_cli_create.check_wallet_address_balance(self.entry_credit_address100000)
            # time.sleep(5)
            os.remove(path)
        # time.sleep(30)
