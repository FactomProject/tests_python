...
class CLITestsChains(unittest.TestCase):
    ...
    def test_check_chain_height(self):
        ...
        seq = self.factom_chain_object.parse_block_data(text)['SequenceNumber']
        ...
        prevMR = self.factom_chain_object.parse_block_data(text)['PrevBlockKeyMR']

        # compare to previous merkel root given by looking up directory block merkel root
        keyMR = self.factom_chain_object.parse_block_data(text)['DBlock']
        text = self.factom_chain_object.get_directory_block(keyMR)
        self.assertTrue(prevMR == self.factom_chain_object.parse_block_data(text)[
